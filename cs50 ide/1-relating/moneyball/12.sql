...
-- INNER JOIN(
--     SELECT players.id AS "player_id", (salaries.salary/performances.RBI) AS "dollars per RBI" FROM players
--     JOIN performances ON performances.player_id == players.id
--     JOIN salaries ON salaries.player_id == players.id
--     AND performances.year == salaries.year
--     WHERE performances.year == 2001 AND performances.RBI > 0
--     ORDER BY "dollars per RBI" ASC, first_name ASC, last_name ASC LIMIT 10
-- ) AS "rbi"
-- ON rbi.player_id == players.id
-- INNER JOIN (
--     SELECT players.id AS "player_id", (salaries.salary/performances.H) AS "dollars per hit" FROM players
--     JOIN performances ON performances.player_id == players.id
--     JOIN salaries ON salaries.player_id == players.id
--     AND performances.year == salaries.year
...
-- ) AS "hit"
-- ON hit.player_id == players.id
-- ORDER BY last_name ASC;

SELECT first_name, last_name FROM (
    SELECT first_name, last_name, id FROM (
        SELECT players.first_name, players.last_name, players.id FROM performances
